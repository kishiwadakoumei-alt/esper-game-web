...
- 相手の手札は枚数だけ公開する。
- ゲーム終了時のみ相手の手札を公開する。
- 山札は通常時は枚数だけ公開し、ゲーム終了時のみ残り山札を上から順に公開する。
- 除外カードはゲーム終了時のみ公開する。
- 自分の裏向き捨て札はカード名を公開する。
- 相手の裏向き捨て札は通常時は `null` とし、表向きカードだけ名前を公開する。
- ゲーム終了時は双方の捨て札名を公開する。ただし `is_face_up` は保持し、元裏向きだったカードをUIで区別できるようにする。
- 各プレイヤーのターン開始回数を `game.turn_counts` で公開し、ターン開始通知の表示時間調整に使う。
...
ゲーム終了後は盤面確認のため捨て札名と残り山札順を公開するが、通知イベントの秘匿方針は通常時の状態に従う。
...
- ゲーム終了時は再戦と退出だけを返す。
- 終了時の `game.result.end_trigger` で、山札切れや捨て札上限などの終了理由を公開する。
...
- 通常時の相手手札、山札内容、除外カード
- 表向き・裏向き捨て札
- ゲーム終了時の相手手札、残り山札順、除外カード、双方の捨て札公開
- サイコキネシス
- ヒーリング
- クレヤボヤンス
- プリサイエンス
