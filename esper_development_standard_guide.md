...
- カード、手札、捨て札、山札枚数の描画
- `available_actions` と `interaction` から盤面操作を有効化
- ターン終了や対象確定を行う固定操作バー
- 能力・宣言・結果・捨て札閲覧のモーダル
- プリサイエンスの順序選択
...
常設のアクション欄は設けない。操作は盤面直接操作、必要時のモーダル、
画面下部の固定操作バーへ分ける。自分の裏向き捨て札も盤面では裏面として
描画し、通常時だけ閲覧専用モーダルで内容を確認できる。
