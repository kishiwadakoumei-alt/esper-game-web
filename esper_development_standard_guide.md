...
- ゲーム終了時の情報公開
- 閲覧者基準の勝利・敗北・引き分けを表す `game.result` の生成
...
- トースト表示
- 保存済みセッションからの復帰
- バトルログ、チャット、セッション情報パネルの開閉
- `?room=` からの招待ルーム事前入力と招待案内
- トップ画面のあいことばを使った招待URL生成と、Web Share API・クリップボードによる共有
- 横画面の捨て札モーダルを画面直下の表示レイヤーへ一時移動
- 画面向き変更時の捨て札表示切り替え
- `renderGame()` への状態引き渡し
- WebSocket接続の開始
...
- カード、手札、捨て札、山札枚数の描画
- 能力別SVGカードとカード詳細の描画
- 自分の裏向き捨て札の裏面表示と閲覧モーダル
- `available_actions` と `interaction` から盤面操作を有効化
- ターン終了や対象確定を行う状況別操作バー
- 能力・宣言・結果・捨て札閲覧のモーダル
- 勝利・敗北・引き分けの結果演出
- プリサイエンスの順序選択と、選択中・確認中の現在手札プレビュー
...
画面下部の状況別操作バーへ分ける。自分の裏向き捨て札も盤面では裏面として
...
- 相手ゾーン、中央ゾーン、自分ゾーンを1画面内へ収める。
- 自分と相手の手札は最大6枚を横一列で表示し、ページの縦スクロールを避ける。
- 横画面ではカード名を省略し、SVGイラストと能力色を主な識別手段にする。
- PCでは利用可能な画面サイズに応じてカードを拡大する。
- 中央ゾーンにはロゴ、山札、ゲーム外カードを配置し、上下の手札と干渉させない。
- セッション情報は右上のボタンから開くスライドパネルへ収納する。
- バトルログとチャットも常設せず、補助ボタンから開閉する。

縦画面では従来どおりカードを折り返し、捨て札を盤面内へ表示する。
横画面専用の捨て札確認ボタンは縦画面では表示しない。

### 8.7 カードと捨て札の表示規則

7能力のカードイラストは `frontend/static/assets/cards/` のSVGを使用し、
`render.js` の `CARD_ART_FILES` でカード名とファイル名を対応付ける。
能力別カラーはカード、選択状態、発光、詳細表示で一貫して使用する。

表示可能なカードは、PCではホバーまたはキーボードフォーカス、
タッチ端末では550ミリ秒の長押しで能力名と効果を表示する。
裏向きカードにはカード名や詳細表示用の属性を付けない。

自分の裏向き捨て札も、相手から見える状態と一致するよう盤面では裏面表示する。
所有者はタップして閲覧専用モーダルを開き、カード名と効果を確認できる。
これは表示上の秘匿であり、所有者向け公開状態にはカード名を含めてよい。
相手向け公開状態には引き続きカード名を含めない。

横画面の捨て札一覧は次の規則で表示する。

- 自分・相手とも画面中央のモーダルとして表示する。
- 変形された盤面要素の座標系に影響されないよう、開いている間だけ
  `app.js` がモーダルを `body` 直下へ移動し、閉じると元の位置へ戻す。
- 1行6グループまで表示し、収まらない場合は改行と縦スクロールを使用する。
- 自分側はシアン、相手側は赤をアクセントカラーにする。
- 状況別操作バーや通知より前面に表示する。
- 捨て札を対象とする能力の選択を確定した後は自動的に閉じる。

カモフラージュなど複数枚が重なる捨て札グループは、通常時は重ねたまま表示し、
展開用のプラス・マイナスボタンを表示しない。複数枚から個別カードを選ぶ必要が
あるヒーリングの選択中だけ展開操作を有効にし、最初のタップまたはプラスボタンで
展開する。展開後はマイナスボタンで閉じられる。能力の確定・キャンセル後は
自動的に閉じて展開ボタンを非表示にする。盤面カードを対象とする能力でも、
最終確認モーダルは省略しない。

### 8.8 結果画面

ゲーム終了時は `game.result` を基準に勝利、敗北、引き分けを判定し、
共通の結果オーバーレイを結果別テーマへ切り替える。

- 勝利: 自分の手札で最多のカード種類を代表カードとし、その能力色で発光する。
- 敗北: 赤紫を基調としたひび割れ、明滅演出を使用する。
- 引き分け: シアンと紫の均衡演出を使用し、両者の代表カードを表示する。

プレイ中は現在の手順に合わせて、状況別操作バーに短い補助文を表示する。
結果画面では勝者の成立能力に合わせて短い勝敗コピーを切り替える。
`WIN CONDITION` の近くに `END TRIGGER` を表示し、山札切れは `DECK OUT`、捨て札上限は `DISCARD LIMIT` として区別する。
結果画面には双方の代表手札枚数を表示し、手札構成による判定理由を具体化する。
ESPER未成立の手札構成勝利では、`ESPER ACHIEVED` ではなく `HAND VERDICT` と判定勝利/判定敗北を表示する。
カード表示は3枚に丸めず、勝者のESPER成立に使われた手札をすべて表示する。
敗北側の画面でも、相手が勝った成立手札を表示する。
引き分け時は双方のESPER成立手札、ESPER未成立なら双方の最多カード群を表示する。
結果画面からは再戦、結果確認、退出を実行できる。結果確認は別モーダルではなく
通常盤面へ戻る操作であり、終了後の盤面では相手手札、残り山札順、ゲーム外カード、
双方の捨て札を公開する。元は裏向きだった捨て札には「元裏向き」表示を付ける。
残り山札は山札を押した時にモーダルで上から順に1枚ずつ表示する。
結果確認中の状況別操作バーからは、結果画面へ戻る操作と再戦操作を実行できる。
終盤は山札残り5枚以下、または捨て札上限まであと3組以下で警告文を表示する。
能力カードは色だけでなく、カード名とアイコンで識別できる。
`prefers-reduced-motion: reduce` が有効な環境ではアニメーション時間を短縮する。
...
招待URLは `?room=<あいことば>` の形式を使用する。受信側は部屋コードを
入力欄へ反映するだけで、参加ボタンが押されるまで入室APIを呼ばない。
共有URLを生成するときは既存のクエリとフラグメントを消去し、`room` だけを
`URLSearchParams` で設定する。BearerトークンはURLへ含めない。
...
人間の操作では、手札を捨てさせた後に `STEP 2 / 2` の案内モーダルを一度だけ
表示する。モーダルから相手の捨て札一覧を開き、選択可能な裏向きカードだけを
サイコキネシスのピンクで発光させる。対象外のカードは暗く表示する。
捨て札一覧を閉じた場合も、状況別操作バーから再度開けるようにする。
最初の効果はすでに解決済みのため、案内モーダルには能力中止ボタンを置かない。

### 12.4 プリサイエンス（未来予知）

1. 山札の上から最大3枚を取り出す。
2. 発動者が上から1枚目、2枚目、3枚目の順序を指定する。
   選択中と最終確認中は、順序を考える参考として現在の自分の手札を表示する。
...
決着時はログ文面だけでなく、`EsperGame` に構造化した結果を保持する。

- `winner_role`: 勝者の `p1` または `p2`。引き分けの場合は `None`
- `result_reason`: `ESPERを宣言`、`ESPER達成`、`手札構成`、
  `双方がESPER達成`、`完全引き分け` などの決着理由
- `end_trigger_reason`: `ESPERを宣言`、`山札が尽きました`、
  `捨て札が18組（上限）に達しました` など、勝敗判定へ入った終了理由

ESPER宣言と自動決着のどちらでもこれらを設定し、再戦初期化時に消去する。
`StateService` は終了時だけ、閲覧者基準の次の結果を `game.result` として返す。
...
```json
{
  ...
  "is_draw": false,
  "reason": "ESPERを宣言",
  "end_trigger": "ESPERを宣言"
}
```

フロントエンドはログ文面を解析して勝敗を判定してはならない。
`is_winner` と `is_draw` を使い、勝利・敗北・引き分けの表示を切り替える。
...
| 自分の手札 | カード名を公開 | 公開 |
| 相手の手札 | 枚数のみ | カード名を公開 |
| 山札 | 枚数のみ | 残り山札を上から順に公開 |
| ゲーム外カード | `null` | カード名を公開 |
| 自分の裏向き捨て札 | 所有者向けデータではカード名を公開。盤面表示は裏面 | カード名を公開し、盤面では表向き。「元裏向き」として区別 |
| 相手の裏向き捨て札 | `null` | カード名を公開し、盤面では表向き。「元裏向き」として区別 |
| 表向き捨て札 | カード名を公開 | 公開 |
| `game.result` | `null` | 勝者、閲覧者の勝敗、引き分け、決着理由、終了理由を公開 |
...
所有者が自分の裏向き捨て札名を知っていることと、盤面で表向きに表示することは
別の要件として扱う。盤面では `is_face_up` が `false` のカードを所有者側でも
裏面表示し、相手が見ているカードの向きを再現する。

所有者が内容を確認する操作はローカルの閲覧モーダルだけで完結し、
カードの向き、公開状態、ゲームロジックを変更しない。相手の裏向きカードは
通常表示、カード詳細、DOM属性のいずれからも名前を取得できない状態を維持する。

ゲーム終了後は例外として、公開済み盤面の確認を目的に残り山札順と双方の捨て札名を表示する。
ただし `is_face_up=false` の情報は残し、元は裏向きだったカードを視覚的に区別する。
...
ターン交代時は中央通知で「あなたの番です」または「相手の番です」を表示する。
自分の通常ターン開始通知には、次の行動を2段階のガイドとして併記する。
各プレイヤーの3ターン目以降は同じ案内の表示時間を短縮する。

1. 手札からカードを1枚選んで捨てる。
2. 山札からカードを引く。

「相手がターンを終了しました」のように直後のターン通知と内容が重複する
イベントは表示しない。
...
- モーダル確定前にAPIを実行しない。
- 横画面でDOM要素を `body` 直下へ移動する場合は、閉じるときと画面向き変更時に
  元の親要素へ戻す。
- 裏向きカードにはカード名や詳細表示用の属性を付けない。
...
- 横画面ではページ全体をスクロールさせず、モーダル内部だけを必要に応じて
  スクロールさせる。
- 横画面と縦画面で同じコンポーネントの配置方法が異なる場合は、両方を確認する。
- 長いプレイヤー名、カード名、ルームIDでレイアウトが崩れないようにする。
- `z-index` を増やす場合は、状況別操作バー、捨て札モーダル、中央通知、
  結果オーバーレイの重なりを確認する。
- 動きの大きい演出には `prefers-reduced-motion` の代替表示を用意する。
...
| `test_frontend.py` | HTML/CSS/JS分離、盤面直接操作、レスポンシブ、捨て札モーダル、カード資産、選択同期、通知、結果演出 |
...
- モーダルの戻る・確定が正しく動く。
- 盤面で選択したカードと状況別操作バーの選択数が一致する。
- サイコキネシスで手札を捨てさせた後、STEP 2案内から相手の捨て札を開ける。
- サイコキネシスの候補だけが発光し、一覧を閉じても操作バーから再表示できる。
- 相手の伏せカード名が開発者ツールのレスポンスにも含まれない。
- ターン交代、相手行動、手札変更、タイムリープが中央表示される。
- 通知が重複せず順番に消える。
- チャット、バトルログ、再戦、退出が動作する。
- 再読み込み後に同じタブのセッションへ復帰する。
- 招待URLを共有でき、別端末で開くと同じあいことばが事前入力される。
- 招待URLを開いただけでは入室せず、参加操作後に2人目として入室する。
- 招待URLにBearerトークンが含まれない。
- 横画面で自分・相手の手札6枚が一列に収まり、ページ全体が縦スクロールしない。
- 自分・相手の捨て札モーダルが画面中央に表示され、一覧が多い場合だけ内部を
  スクロールできる。
- 相手の捨て札モーダルは赤、自分の捨て札モーダルはシアンで表示される。
- 自分の裏向き捨て札は通常時の盤面で裏面になり、タップしたときだけ内容を確認できる。
- 終了後の盤面では双方の裏向きだった捨て札も表向きになり、「元裏向き」と分かる。
- 終了後に山札が残っていれば、山札モーダルで上から底までの順番を確認できる。
- 表向きカードはPCのホバー、スマートフォンの長押しで詳細を確認できる。
- 勝利、敗北、引き分けの3経路で結果演出、盤面での結果確認、結果画面へ戻る、再戦、退出が動作する。
