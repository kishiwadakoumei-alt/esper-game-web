...
| 自分の裏向き捨て札 | 所有者向けデータではカード名を公開。盤面表示は裏面 | 公開状態は同じ。盤面表示は裏面 |
| 相手の裏向き捨て札 | `null` | `null`のまま裏向きを維持 |
| 表向き捨て札 | カード名を公開 | 公開 |
...
- 自分の裏向き捨て札は盤面で裏面になり、タップしたときだけ内容を確認できる。
- 表向きカードはPCのホバー、スマートフォンの長押しで詳細を確認できる。
- 勝利、敗北、引き分けの3経路で結果演出、再戦、結果確認、退出が動作する。
